...
function showPerm(id, roomId, proxy, proxyId) {
	...
	parent.detailFrame.location = url + CsrfGuard.getUrlSurffix();
}

<!-- 中国石油天然气股份有限公司西南油气田分公司  【增加导出功能】  lixuqiang 2020年4月29日 start -->                 
function doExport() {
...
<tr>
	<td class="webfx-menu-bar" style="height:30px;">
		<script type="text/javascript">
			...
			<!-- 中国石油天然气股份有限公司西南油气田分公司  【增加导出功能】  lixuqiang 2020年4月29日 start -->                 
		    myBar.add(new WebFXMenuButton("modify", "导出", "doExport()", [2,8]));
		    <!-- 中国石油天然气股份有限公司西南油气田分公司  【增加导出功能】  lixuqiang 2020年4月29日 end -->                 
	...
	<td class="webfx-menu-bar" style="height:30px;">
		<form id="searchForm" name="searchForm" method="post" onsubmit="return false" action="meetingroomList.do?method=listPerm&=all&list=waitReview${ctp:csrfSuffix()}">
			<input type="hidden" name="flag" value="${v3x:toHTML(flag)}"/>
			<input type="hidden" name="doExecl" id="doExecl" value="${currentExecl}"/>
...
<script type="text/javascript">
showCondition("${v3x:escapeJavascript(param.condition)}", "${v3x:escapeJavascript(param.textfield)}", "${v3x:escapeJavascript(param.textfield1)}");
